...
mkdir -p /tmp/kaniko
cd /tmp/kaniko
url="https://ghfast.top/https://github.com/k8stooling/kaniko-standalone/releases/download/v1.23.2/kaniko-binaries-linux-${platform}.scrambled"
...
sudo chroot . bash -c ". /.env; set;cat $DOCKER_CONFIG/config.json; ./kaniko/executor -f /kaniko/workspace/$(basename ${dockerfile}) --context=/kaniko/workspace/ --force --destination=$destination --cleanup $KANIKO_EXTRA_ARGS"
